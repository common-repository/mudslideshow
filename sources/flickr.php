...
function mudsFlickr_check_user($user) {
	...
	if(!preg_match($search, $user)) { //It doesn't look like a Flickr ID
		...
		if(preg_match($search, $user)) { //Is an email?
			$url = "https://api.flickr.com/services/rest/?method=flickr.people.findByEmail&api_key=". MUDS_FLICKR_API ."&find_email=".$user;
		} else { //It's a username
			$url = "https://api.flickr.com/services/rest/?method=flickr.people.findByUsername&api_key=". MUDS_FLICKR_API ."&username=".$user;
		}
...
function mudsFlickr_get_username($userid) {
	$answer = $userid;
	$url = "https://api.flickr.com/services/rest/?method=flickr.people.getInfo&api_key=". MUDS_FLICKR_API ."&user_id=$userid";
...
function mudsFlickr_update_user_galleries_list($user) {
	...
	//albums he has?
	$url = "https://api.flickr.com/services/rest/?method=flickr.photosets.getList&api_key=". MUDS_FLICKR_API ."&user_id=$user";
	if($data = muds_readfile($url)) { //Ask flickr
		if($data[0]=='<') { //Ok, we have an answer and it looks like an XML
			...
			if($attr->stat=='ok') { //The answer is correct 
				...
				$gallery_url = "https://www.flickr.com/photos/$user/";
				...
				foreach($data->photosets->photoset as $entry) { //Read each entry
					...
					$gallery_url = "https://www.flickr.com/photos/$user/sets/$album/";
				...
				$url = "https://api.flickr.com/services/rest/?method=flickr.tags.getListUser&api_key=". MUDS_FLICKR_API ."&user_id=$user";
				if($data = muds_readfile($url)) { //Ask flickr
					$data = new SimpleXMLElement($data); //Parse the XML
					foreach($data->who->tags->tag as $entry) { //Read each entry
						...
						$gallery_url = "https://www.flickr.com/photos/$user/tags/$tag/";
...
function mudsFlickr_update_zero( $user ) {
	...
	$url = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=". MUDS_FLICKR_API . "&user_id=$user&per_page=$per_page&extras=url_t,url_m,url_z,url_l,url_o,description";
	...
	if($data = muds_readfile($url)) { //Ask flickr
		...
		if($attr->stat=='ok') { //The answer is correct 
			...
			foreach($data->photos->photo as $entry) {
				...
				$photo_url = "https://www.flickr.com/photos/$user/$id/";
...
function mudsFlickr_update_album( $user, $album ) {
	...
	if($album>0) { //If it is a number
		//Hello flickr, we again. Can you show the photos from this album?
		$url = "https://api.flickr.com/services/rest/?method=flickr.photosets.getInfo&api_key=". MUDS_FLICKR_API . "&photoset_id=$album";
		...
		if($data = muds_readfile($url)) { //Ask flickr
			...
			if($attr->stat=='ok') { //The answer is correct 
				...
				$url = "https://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=". MUDS_FLICKR_API . "&photoset_id=$album&extras=url_t,url_m,url_z,url_l,url_o,description";
				...
				foreach($data->photoset->photo as $entry) {
					...
					$photo_url = "https://www.flickr.com/photos/$user/$id/in/set-$album/";
...
function mudsFlickr_update_tag( $user, $tag ) {
	...
	$url = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=". MUDS_FLICKR_API . "&user_id=$user&tags=".urlencode($tag)."&extras=url_t,url_m,url_z,url_l,url_o,description";
	$gallery_title=__('Album doesn\'t exist', 'mudslide' ); //Maybe we are wrong, so this is to create a dummy answer
	$out=false;
	if($data = muds_readfile($url)) { //Ask flickr
		$data = new SimpleXMLElement($data); //Parse the XML
...
function mudsFlickr_update_feed( $url ) {
	...
	$md5 = md5($url);
	$gallery_title=__('Album doesn\'t exist', 'mudslide' ); //Maybe we are wrong, so this is to create a dummy answer
	if(class_exists('SimplePie') && strstr($url, "https://api.flickr.com") ) { // Does simplepie library exists? Is this a Flickr feed?
